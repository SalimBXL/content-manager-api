...
app.get("/api/resources/:id", (req, res) => {
    const {id} = req.params;
    const resources = getResources();
    const resource = resources.find(resource => id === resource.id);
    res.send(resource);
})
...
app.patch("/api/resources/:id", (req, res) => {
    const {id} = req.params;
    const resources = getResources();
    const index = resources.findIndex(resource => id === resource.id);
    const activeResource = resources.find(resource => resource.status === "active");

    if (resources[index].status === "complete") {
        return res.status(422).send("Cannot update a resource already completed.");
    }
